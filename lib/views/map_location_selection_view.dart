...
class _MapLocationSelectionViewState extends State<MapLocationSelectionView> {
  ...
  late final MapLocationProvider _provider;

  @override
  void initState() {
    super.initState();
    _provider = MapLocationProvider();
    _provider.addListener(_onProviderChanged);

    // 初期化
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _provider.initializeMap(
  ...
  void _onProviderChanged() {
    if (_textEditingController.text != _provider.locationName) {
          ...
          child: Column(
            children: [
              ...
              Expanded(
                child: Stack(
                  children: [
                    // GoogleMap - 常に表示して再作成を防ぐ
                    Consumer<MapLocationProvider>(
                      builder: (context, provider, child) {
                        return GoogleMap(
                          initialCameraPosition: CameraPosition(
                            target: provider.selectedLocation ?? const LatLng(35.6762, 139.6503), // デフォルト位置
                            zoom: 15,
                          ),
